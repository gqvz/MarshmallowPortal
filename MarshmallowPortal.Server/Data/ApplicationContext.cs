    ...
    public async Task<User> LoginUser(string code, TokenType type)
    {
        switch (type)
        {
            case TokenType.Google:
                var user0 = await _googleGoogleOAuth2Service.GetUser(code, "http://localhost:6001/google");
                User u0 = user0;
                u0.TokenLifetime = 3599;
                _users.Add(u0);
                return u0;
            ...
            case TokenType.Discord:
                var user2 = await _discordGoogleOAuth2Service.GetUser(code, "http://localhost:6001/discord");
                User u2 = user2;
                u2.TokenLifetime = 604799;
                _users.Add(u2);
                return u2;
            default:
